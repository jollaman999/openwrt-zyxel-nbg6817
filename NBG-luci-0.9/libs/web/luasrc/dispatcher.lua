...
function authenticator.htmlauth(validator, accs, default)
	...
	if block then
		sys.exec("rm /tmp/login_info")
		sys.exec("echo 'block' > /tmp/login_state")
		luci.http.redirect(luci.dispatcher.build_url() .. "?login_error=1")

		sys.exec("/bin/block_time")--count block time
	end
	
	if user and validator(user, pass) then
		local block_state=sys.exec("cat /tmp/login_state | tr -d '\n'")
		if block_state ~= "block" then
			sys.exec("rm /tmp/login_info")
		return user
		else
			luci.http.redirect(luci.dispatcher.build_url() .. "?login_error=1")
    ...
    -- Modify "Password Visible in Source Code" for Login, NBG6816, WenHsiang, 2012/05/10	
    if user then
		sys.exec("echo 'login enter info' >> /tmp/login_info")
		luci.http.redirect(luci.dispatcher.build_url() .. "?login_error=1")
    end
	user_pass_num=sys.exec("cat /tmp/login_info | wc -l | tr -d '\n'")
    --
...
--for get weather, NBG6816, john, 2011/03/25
...
--for check weather, NBG6816, john, 2011/03/25
...
--Change Language for eaZy123, NBG6816, WenHsiang, 2011/12/09
...
--for applying weather and time immidiately, NBG6816, john, 2011/03/21	 
