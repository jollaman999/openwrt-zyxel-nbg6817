...
function net.fw_service(callback)
	...
	for s in pairs(sections_or) do
		if not (s == "general" or s == "remote_www" or s == "remote_telnet" or s == "qcanssecm" or s == "remote_https" or s == "remote_ssh") then
...
function net.sendto(callback)
	local index = 1
	local emails = { }
	local sections = {}
	local sections_or = uci:get_all("sendmail")
	--local num = table.getn(sections_or)

	
	for s in pairs(sections_or) do
		if not (s == "mail_server_setup") then
			local num = tonumber(s:match("%d+"))
			sections[num]=s
		end
	end
	...
	for i,k in pairs(sections) do	
		--if not (k == "mail_server_setup") then
			local rt = uci:get_all("sendmail",k)
			if rt.email then 
				rt["index"] = index
				rt["rule"] = k
				if callback then
...
function net.porttrigger()
	...
	local sections = uci:get_all("portTrigger")
		for i in pairs(sections) do	
			...
			if rt then	
				if i ~= "general" then		
					local rule_name=uci:get("portTrigger",i,"rules_name")
					rt["trigger_name"] = rule_name
					table.insert(t,rt)
				end
			end	
...
local index = 1;
	for line in io.lines("/tmp/dhcp.leases") do
		...
		local sNum, macAdd, ipAdd, hostName,macAdd2 = line:match(
			...
			-- "^%s" : All non-space characters, NBG6816, WenHsiang, 2012/05/02
		)
...
function net.upnp(callback)
	local routes = { }
	local index = 1;
	for line in io.lines("/tmp/upnp") do

		local  prtocal,inPort,ipAdd,exPort = line:match(
			"(%a+) (%d+) ([a-zA-F0-9.*]+) (%d+)")
...
function net.obtainDns(callback)
	...
	local index = 1;
        for line in io.lines("/tmp/resolv.conf.auto") do
                local DNS_IP = line:match("%d+%.%d+%.%d+%.%d+")
                if DNS_IP then
                        local rt = {
                                index    = index,
                                dns    = DNS_IP
