...
# for check out GUI from git server
define DownloadMethod/git_repo
	@echo "Checking out $(FILE) from the git repository..."; \
	mkdir -p $(TOPDIR) && cd $(TOPDIR) && \
	git clone $(URL) $(SUBDIR) --recursive && \
		(cd $(SUBDIR) && git checkout $(VERSION)) 
endef

# for check out SRC from SVN server
define DownloadMethod/svn_src
	$(call wrap_mirror, \
		echo "Checking out source files from the svn repository..."; \
		mkdir -p $(DL_DIR) && cd $(DL_DIR) && \
		[ \! -d $(SUBDIR) ] && \
		( svn help checkout | grep -q trust-server-cert && \
		svn checkout --non-interactive --trust-server-cert -r$(VERSION) $(URL) $(SUBDIR) || \
		svn checkout --non-interactive -r$(VERSION) $(URL) $(SUBDIR) ) ;
	)
endef
