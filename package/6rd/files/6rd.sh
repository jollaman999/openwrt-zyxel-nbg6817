...
#	local macGuest=$(ifconfig br-lan-guest | sed -ne 's/[[:space:]]*$//; s/.*HWaddr //p')
#	local ck6rdGuestChange="0"
#	local ck6rdConfig=$(cat /var/etc/radvd_6rd.conf |grep guest)
#	[ -n "$macGuest" ] && {
...
#
#			ifconfig br-lan-guest down
#			[ -n "$ipv6lanGuestold" -a "$ipv6lanGuestold" != "$ipv6lanGuest" ] && ifconfig br-lan-guest del "$ipv6lanGuestold"/64
#			ifconfig br-lan-guest add "$ipv6lanGuest"/64
#			ifconfig br-lan-guest up
