...
local function secondsOffsetFromTimeZone(timezoneOffset)
    if string.len(timezoneOffset) ~= 5 then
        return 0
    end

    local sign = string.sub(timezoneOffset, 1, 1)
    local hours = tonumber(string.sub(timezoneOffset, 2, 3))
    local minutes = tonumber(string.sub(timezoneOffset, 4, 5))
...
local function appendCurrentNodeInfoWithNodeHistory(currentTimestamp, accOfIndex)
    ...
    local patchedNodeHistory = calculateDeviceDataWithIndex(lowerBoundIndex, higherBoundIndex, startTimestamp, endTimestamp, currentTimestamp, 0)
...
local function jsonStringOfHistoryInTR069Format(isAppendRealtimeData, currentTimestamp, accOfIndex)
	...
	if isAppendRealtimeData == 1 then
		retrivedNodeHistory = appendCurrentNodeInfoWithNodeHistory(currentTimestamp, accOfIndex)
...
local function fetchNodeHistory()
	...
	return jsonStringOfHistoryInTR069Format(isAppendRealtimeData, currentTimestamp, accOfIndex)
