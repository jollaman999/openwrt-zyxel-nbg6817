...
angular.module('ozkerDebug.directives', ['stats'])
    ...
    */
    ...
    .directive('fxNetwork', [function () {
        return {
            restrict: 'E',
            template: '<div id="Display" style="width:800px;height:100%;"></div>',
            link: function(scope, element, attrs) {
                ...
                function handler(icon,name,x,y,node,id)
                {
                    ...
                    if(n==-1)
                    {
                        pathname = pathname.replace(/streamboost_fxnetwork/g,"streamboost_fxnode")
                    }
                    else
                    {
                        pathname = pathname.replace(/streamboost_fxnetwork/g,"streamboost_fxnode")
    ...
    */
    ...
    .directive('fxNode', [function () {
        return {
            restrict: 'E',
            template: '<div id="Display" style="width:800px;height:100%;min-height:768px;"></div>',
            link: function(scope, element, attrs) {
    ...
    */
    ...
    .directive('fxTopByTime',['$http', 'stats', function ($http, stats) {
        return {
            restrict: 'E',
            template:
                '<div class="chartOptions" style="float: right; margin-top: -64px; margin-right: 50px;">'+
                '  <div>'+
                ...
                '  </div>'+
                '</div>'+
                // '<div id="chart_div_withoptions" style="width: 800px; min-height: 600px; margin-left:auto; margin-right:120px; margin-top:64px;"></div>',
                '<div id="chart_div_withoptions" style="width: 800px; min-height: 600px; margin:auto; "></div>',
            link: function(scope, element, attrs) {
                ...
                function handleFilterChange() {
                  ...
                  g_nEnd = new Date(getCurrentTime());
                  g_nStart = new Date(g_nEnd - msecs[dateRange.val()]);
            //      $('.chartTitle').text("Top 5 Flows In The " + dateRange.text() + " For " + node.text());

                    stats.getFlowsTime(g_nStart,g_nEnd,node.val(), g_nRemainingEventFlows - g_nNumOfEventFlowPerFetch, g_nRemainingEventFlows)
                    .then(graphTopByTimeCallback);
                ...
                var graphTopByTimeCallback = function(topData) {
                ...
                function graphTopByTime(start,end,node) {
                    ...
                    // console.log('1. getFlowsTime');
                    $("body").addClass("loading");  
                    $("#divModal").show();

                    // console.log('current g_nRemainingEventFlows: '+ g_nRemainingEventFlows);
                    stats.getFlowsTime(start,end,node,g_nRemainingEventFlows - g_nNumOfEventFlowPerFetch, g_nRemainingEventFlows)
                    .then(graphTopByTimeCallback);
                }
            }
    ...
    */
    ...
    .directive('fxTopDownloads', ['stats', function (stats) {
        return {
            restrict: 'E',
            //template: '<div id="chart_div" style="width: 800px; min-height: 600px; margin-left:auto; margin-right:120px;"></div>',
            template: '<div id="chart_div" style="width: 800px; min-height: 600px; margin:auto; "></div>',
            link: function(scope, element, attrs) {
                ...
                var graphDownloadCallback = function(topData) {
                    // console.log('(start, end): ' + g_nStart + ', ' + g_nEnd);
                    // console.log('topData :');
                    // console.log(topData);
                    if (scope.chart) 
                    ...
                    if (g_nRemainingEventFlows > 0)
                    {
                        ...
                        stats.getFlowsTime(g_nStart, g_nEnd, null, g_nRemainingEventFlows - g_nNumOfEventFlowPerFetch, g_nRemainingEventFlows)
                        .then(graphDownloadCallback);
