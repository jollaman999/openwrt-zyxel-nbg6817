...
define KernelPackage/ipt-webstr
  SUBMENU:=$(NF_MENU)
  TITLE:=Module for WEBSTR
  KCONFIG:=$(KCONFIG_IPT_WEBSTR)
  FILES:=$(foreach mod,$(IPT_WEBSTR-m),$(LINUX_DIR)/net/$(mod).ko)
...
define KernelPackage/ipt-webstr/description
 Netfilter (IPv4) module for WEBSTR
 Includes:
 - webstr
endef
...
define KernelPackage/ipt-porttrigger
  SUBMENU:=$(NF_MENU)
  TITLE:=Module for PORTTRIGGER
  KCONFIG:=$(KCONFIG_IPT_PORTTRIGGER)
  FILES:=$(foreach mod,$(IPT_PORTTRIGGER-m),$(LINUX_DIR)/net/$(mod).ko)
  AUTOLOAD:=$(call AutoLoad,45,$(notdir $(IPT_PORTTRIGGER-m)))
  $(call AddDepends/ipt)
  DEPENDS:= kmod-ipt-core
endef

define KernelPackage/ipt-porttrigger/description
 Netfilter (IPv4) module for PORTTRIGGER
 Includes:
 - PORTTRIGGER
endef

$(eval $(call KernelPackage,ipt-porttrigger))
