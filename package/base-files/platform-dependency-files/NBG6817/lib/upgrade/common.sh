...
run_ramfs() { # <command> [...]
	install_bin /bin/busybox /bin/ash /bin/sh /bin/mount /bin/umount        \
		...
		/bin/sed /bin/rm /bin/echo /bin/ps

	install_bin /sbin/mtd
	install_bin /bin/fw_upgrade
	for file in $RAMFS_COPY_BIN; do
		install_bin $file
	done
	install_file /etc/config/* $RAMFS_COPY_DATA
...
kill_remaining() { # [ <signal> ]
	...
	for stat in /proc/[0-9]*/stat; do
		...
		case "$name" in
			...
			*ash*|*init*|*watchdog*|*ssh*|*dropbear*|*telnet*|*login*|*hostapd*|*wpa_supplicant*|*logger*|*EmappS*) : ;;
