...
#include <mmc.h>
#define MMC_FLASH_DEVICE_INDEX      0
int zmmc_init(void)
{
	return 0;
}
int zmmc_read(unsigned long blk, unsigned long cnt, char *buf)
{
	...
	mmc = find_mmc_device(MMC_FLASH_DEVICE_INDEX);
	if (!mmc)
		return 1;
	...
	printf("\nMMC read: dev # %d, block # %d, count %d ... \n",
				MMC_FLASH_DEVICE_INDEX, blk, cnt);
	n = mmc->block_dev.block_read(MMC_FLASH_DEVICE_INDEX, blk, cnt, buf);
	flush_cache((ulong)buf, cnt*512);
...
int zmmc_write(unsigned long blk, unsigned long cnt, char *buf)
{
	...
	printf("\nMMC write: dev # %d, block # %d, count %d ... \n",
				MMC_FLASH_DEVICE_INDEX, blk, cnt);
	n = mmc->block_dev.block_write(MMC_FLASH_DEVICE_INDEX, blk, cnt, buf);
...
int zmmc_erase(unsigned long blk, unsigned long cnt)
{
	...
	printf("\nMMC erase: dev # %d, block # %d, count %d ... \n",
				MMC_FLASH_DEVICE_INDEX, blk, cnt);
	n = mmc->block_dev.block_erase(MMC_FLASH_DEVICE_INDEX, blk, cnt);
...
enum {
    ...
    ATCMD_FLASH_TYPE_MMC,
    ATCMD_FLASH_MAX_TYPE
};
static unsigned int zflash_cmd_type=0; // 0-NOR flash, 1-NAND flash, 2-MMC flash
static int do_zflash_cmd(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	printf("zflash_cmd_type = %d\n", zflash_cmd_type);
	switch (zflash_cmd_type) {
	case ATCMD_FLASH_TYPE_MMC:
		printf("MMC flash commands activated\n");
		break;
	default:
		...
		if (zflash_cmd_type == ATCMD_FLASH_TYPE_MMC) {
			if (zmmc_read(offs, len, (char *)addr))
				return -1;
		} else if ( zflash_cmd_type==ATCMD_FLASH_TYPE_NAND?znand_read(offs, len, (char *)addr):zflash_read(offs, len, (char *)addr) ) return -1;
		...
		if (zflash_cmd_type == ATCMD_FLASH_TYPE_MMC) {
			if (zmmc_erase(o, l))
				return -1;
		} else if ( zflash_cmd_type==ATCMD_FLASH_TYPE_NAND?znand_erase(o, 0, l):zflash_erase(o, l) ) return -1;
		...
		len = simple_strtoul(argv[4], NULL, 16);
		if (zflash_cmd_type == ATCMD_FLASH_TYPE_MMC) {
			if (zmmc_write(offs, len, (char *)addr))
				return -1;
		} else if ( zflash_cmd_type==ATCMD_FLASH_TYPE_NAND?znand_write(offs, 0, len, (char *)addr):zflash_write(offs, len, (char *)addr) ) return -1;
	} else {
zflash_cmd_failed:
...
U_BOOT_CMD (
	...
	"Flash access commands, length must be align to block/page size",
        "switch <0-NOR|1-NAND|2-MMC>                   - switch flash type for commands"
    ...
    "  <addr>: memory address\n"
    "  <offs>: flash offset/address/mmc_block#\n"
);
#endif
